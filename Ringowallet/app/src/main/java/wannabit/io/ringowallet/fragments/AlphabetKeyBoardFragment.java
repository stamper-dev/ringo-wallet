...
public class AlphabetKeyBoardFragment extends KeyboardFragment implements View.OnClickListener{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        mRootView = inflater.inflate(R.layout.fragment_alphabet_keyboard, container, false);
        ...
        for(int i = 0; i < mAlphabetBtns.length; i++) {
            ...
            mAlphabetBtns[i].setTypeface(WUtils.getTypefaceLight(getBaseActivity()));
    ...
    public void onClick(View view) {
        if(view instanceof Button) {
            if(mListner != null) {
                mListner.onInsertKey(((Button)view).getText().toString().trim().toCharArray()[0]);
            }

        } else if (view instanceof ImageButton) {
            if(mListner != null) {
                mListner.onDeleteKey();
            }
        }
    }
}