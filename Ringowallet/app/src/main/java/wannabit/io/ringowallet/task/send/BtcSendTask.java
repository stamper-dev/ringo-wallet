...
public class BtcSendTask extends SendTask {

    private BaseApplication         mApp;
    private TaskCallback            mCallback;
    private TaskResult              mResult;

    public BtcSendTask(BaseApplication app, TaskCallback mCallback) {
        WLog.w("BtcSendTask");
        this.mApp = app;
...
//    Response<ResBlock> resBlock = ApiClient.getBlock(mApp, resInfo.body().getHeadBlockNum()).execute();
    ...
    protected TaskResult doInBackground(Bundle... bundles) {
        ...
        try {
            ...
            Response<List<ResUTXByAddress>> resUTX = ApiClient.getWannabitService(mApp).getBTCUTXOByAddr(pkey.toAddress(params).toString()).execute();
...
//                Response<ResRawTx> resRawTx = service.getBTCRawTx(resUTX.body().get(i).getTxid()).execute();
...
//            }
            ...
            Response<ResCreateRawTx> rawTxResponse = ApiClient.getWannabitService(mApp).createBTCRawTx(rawNewPropose).execute();
            ...
            Response<ResSendRawTx> mainResult = ApiClient.getWannabitService(mApp).sendBTCRawTx(sendReq).execute();
            ...
            WLog.w("BtcSendTask : "  + e.getMessage());
